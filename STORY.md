...
## 2026-04-16 · Day 1 (iteration 7) — "flat AI" diagnosed as voice-range bottleneck; drama amp response

**Your feedback.** The auditions confirmed voice choice was right (Daisy=af_heart, Gatsby=am_onyx), but the voices still sounded "very AI and flat." The narrator (am_michael) was relatively better — meaning the issue wasn't the pipeline, it was the specific expressiveness of Kokoro's American presets.

**The diagnosis that clicked.** *Engine expressiveness is non-uniform across its own voice library.* I'd been treating "Kokoro" as a single quality tier, but the British preset voices (Isabella, Lewis, Emma, Michael — who happens to be tagged en-US but reads with more prosodic range than Onyx/Heart) have markedly more natural pitch variation than the American presets Onyx/Heart. Same model, same training run, same MLX inference path — different voices = different expressive ranges. This is the engine-specific equivalent of "not all humans act at the same skill level even if they speak the same language."

**What I chose as the response.** Before escalating to a new engine (the Chatterbox-with-voice-cloning option is the nuclear one — a day of LibriVox clipping and reference-hygiene), exhaust the structural-prosody levers:
1. **Widen pace coefficient** in the backend (0.28 → 0.40). Slow lines are now actually slow.
2. **Double the intensity deceleration** for peaks. A line at intensity 0.95 now decelerates ~9× harder than a line at intensity 0.5.
3. **Drama-punctuation hint** to the synth: append `…` to intensity-≥0.90 lines ending in a period. The synth tapers the last word instead of clipping. Invisible to the script (faithful-wording contract protected at the validator boundary).
4. **Doubled pause gains** around peaks — silence is the prosodic lever when pitch range is tapped out.
5. **Widen script contrast** — Gatsby/Daisy peaks pushed to 0.95–1.0, Daisy's deliberately-flat "We haven't met for many years." pushed *down* to 0.35. Contrast is information.

**The bench.** RTF improved slightly to 0.13 (no perf cost for the drama amp), QA 23/23 still clean, Whisper 0.982 (vs 0.985 previous — within noise, drama-ellipsis insertion doesn't materially change what Whisper hears). Audio duration up 3.6 s from the longer held-breath silences, which is the point.

**What this teaches.**
- *Don't generalize engine quality from a single voice.* When one voice in a TTS library sounds flat, A/B it against its siblings on the same content before blaming the engine. Might be a voice-tier problem, not an engine-tier problem.
- *Structural prosody is a free lever for flat voices.* Kokoro's American voices cost us nothing to make more expressive via silence + pace + contrast. We got meaningful dynamic range without swapping engines. The generalization: *when the synth's prosody is fixed, shape the space around it — pace, pauses, contrast, punctuation-hints.*
- *Know your escalation tree and hold the expensive option.* Chatterbox + voice cloning is the next level up, but it costs setup, needs reference clips, adds determinism risk, and introduces a new backend to maintain. Exhausting free levers first = earning the right to spend the expensive one when it's actually needed.
- *Contrast is information.* Daisy at 0.35 → 0.9 across three lines feels more emotional than the same three lines at a constant 0.7, even with the same voice. This is the same principle as musical dynamics — the silence between loud and soft is the art.

**Concept bucket (added).**
- *Voice-tier vs engine-tier quality* — within a single TTS model, voices vary in expressive ceiling. Audition comparisons stay within a voice tier; engine-level decisions happen when *all* voices of a model hit the same ceiling.
- *Structural prosody as free compensation* — pace, pause, contrast, and punctuation-hints can add perceived expressiveness to a flat voice at ~0 cost. Exhaust these before engine swaps.
- *The escalation tree discipline* — always know the next step up, but earn the right to take it by exhausting the cheaper levers first. Documenting the tree in decisions (here: drama-amp → Chatterbox-clone → Sesame-CSM) means future sessions skip the rediscovery.
